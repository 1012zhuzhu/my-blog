...
export async function generateStaticParams(){
    const getFilesPath = path.join(process.cwd(),'posts');

    if(!fs.existsSync(getFilesPath)) return[];

    const filesread = fs.readdirSync(getFilesPath);

    return filesread.filter((file) => file.endsWith('.md')).map((file) => ({
...
function getRecentPosts(currentSlug: string){
    const getPostsPath = path.join(process.cwd(),'posts');
    let fileNames: string[] = [];
    try {
        fileNames = fs.readdirSync(getPostsPath).filter(f => f.endsWith('.md'));
    } catch(e) {}
    if (!fileNames) return [];
    return fileNames.map(f => {
        const s = f.replace(/\.md$/,'');
        const c = fs.readFileSync(path.join(getPostsPath,f),'utf8')
        const { data } = matter(c)
        return {
            slug: s,
            title: data.title || '无标签',
            date: data.date
        }
            ...
            <div className="p-5 md:p-12 relative">
              ...
              <header className="mb-6 md:mb-8 border-b border-slate-300/50 dark:border-slate-700 pb-5 md:pb-6 relative">
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                  <div className="flex items-center gap-1.5 md:gap-2 text-indigo-700 dark:text-indigo-400 font-bold bg-white/30 dark:bg-slate-900/50 px-3 md:px-4 py-1.5 md:py-2 rounded-full w-max text-xs md:text-sm
                  transition-colors duration-700 shadow-sm border border-white/20 dark:border-white/5">
              ...
              <div className="relative">
                ...
                <div
                  id="article-content"
                  className="prose prose-jelly dark:prose-invert prose-base md:prose-lg max-w-none text-slate-800 dark:text-slate-200 transition-colors duration-700 scroll-smooth"
