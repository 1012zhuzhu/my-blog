...
export default function Navbar(){
  ...
  // 在组件内加这一行，跟 smoothRotation 放一起
  const inverseRotation = useTransform(smoothRotation, (v) => -v);
  ...
  return (
    ...
    <header className={`hidden md:block w-full fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${showNav ? 'translate-y-0' : '-translate-y-full'} bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border-white/20 dark:border-white/5 shadow-sm`}>
      <div className="w-[90%] max-w-6xl mx-auto h-16 flex items-center justify-between px-4 sm:px-8 box-border">
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <>
              ...
              />
              <motion.div initial={{ scale: 0.5, opacity:0, rotate: -90}}animate={{ scale: 1, opacity: 1, rotate: 0 }}
                    ...
                    return (
                      ...
                      >
                        <motion.div style={{ rotate: inverseRotation }} className="w-full h-full">
                          ...
                          >
                            <span className="text-[11px] font-black">{link.name}
                            </span>
                          </Link>
                        </motion.div>
                      </div>
