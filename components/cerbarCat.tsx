...
export default function CyberCat() {
  ...
  const handleFeed = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setTimeout(() => speak(data.reply,8000), 6000);
    } catch(error){
        setTimeout(() => speak("吧唧吧唧... 鱼干好吃，但本喵卡壳了喵...",4000), 6000)
    } finally {
      setIsThinking(false)
    }
  }
  const handleInputSubmit = async (e: React.FormEvent) => {
    ...
    const useMessage = inputValue.trim();
    setIsThinking(true);
    setShowInput(false);
    setInputValue('');
    speak('本喵正在思考你的问题...',6000)
    ...
    try {
      ...
      if(!res.ok) throw new Error('接触API错误')
      const data =await res.json();
      setTimeout(() => speak(data.reply,6000), 6000)
    } catch(error){
      setTimeout(() => speak("铲屎官的网线被老鼠咬断了吧？喵！", 4000), 6000)
    } finally{
      setIsThinking(false);
    }
  }
  ...
  useEffect(() =>{
    ...
    const randomTalkInterval = setInterval(() => {
        if(!speech && !showInput && !isThinking && Math.random() > 0.8){
            const talkRandom = randomBarks[Math.floor(Math.random()*randomBarks.length)]
            speak(talkRandom,4000)
        }
    },20000)
    ...
    >
      <div className='relative w-full flex justify-center mb-6'>
          <AnimatePresence>
            {speech && (
              ...
              >
                {speech}
                <div className="bottom-[-6px] left-1/2 -translate-x-1/2 w-3 h-3 bg-white dark:bg-slate-800 border-b border-r border-gray-100 dark:border-slate-700 transform rotate-45"></div>
              </motion.div>
            )}
          </AnimatePresence>
      </div>
      ...
      <div className="relative">
        <div className='absolute -left-12 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-20'>
            ...
            onClick={handleFeed}
            disabled={isThinking}
            className={`bg-white/90 dark:bg-slate-700/90 p-2.5 rounded-full shadow-md hover:scale-110 active:scale-95 transition-transform border border-gray-100 dark:border-slate-600 flex items-center justify-center
            backdrop-blur-sm ${isThinking ? 'opacity-50 cursor-not-allowed' : ''}`}
            title='小鱼干'
            >
        ...
        <div
           className='w-[120px] h-[120px] relative cursor-pointer' onClick={handlePetCat}
        >
          <style>{`
            .cat-sprite {
              width: 100%;
