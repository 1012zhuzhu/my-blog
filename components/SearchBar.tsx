...
export default function SearchBar({posts = []}:{posts:Post[]}) {
   ...
   useEffect(() => {
        const handOutClick = (e : MouseEvent) => {
            if (container.current && !container.current.contains(e.target as Node)) {
                setIsOpen(false)
            }
        //这里设计处理是考虑到了浏览器和react的特性在react里handOutClick已经没有了但是在浏览器里并没有消失浏览器可以接着调用此属性
        }
        document.addEventListener('mousedown',handOutClick)
        return () => document.removeEventListener('mousedown',handOutClick)
   },[])
  ...
  const searchResult = useMemo(() => {
    ...
    return posts.filter(post => {
        ...
        const decMatch = (post.description || '').toLowerCase().includes(query);
        const tagMatch = (post.tags || []).some(tag => tag.toLowerCase().includes(query));

        return titleMatch || decMatch || tagMatch
    })
   },[searchQuery,posts]);
    ...
    <div className='relative w-full max-w-2xl mx-auto mb-10 z-[100]' ref={container}>
        ...
        <AnimatePresence>
            {isOpen && searchQuery.trim() !== '' && (
                ...
                exit={{opacity: 0,y: -10,scale: 0.98}} transition={{duration: 0.2 }} className='absolute top-full left-0 right-0 mt-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl border border-white/50 dark:border-slate-700/50 rounded-3xl shadow-2xl overflow-hidden max-h-[450px] overflow-y-auto z-20'>
                    {searchResult.length > 0 ? 
                    (<div  className ='flex flex-col py-3'>
                        {searchResult.map((post) => (<Link 
                        ...
                        <div className='flex item-start justify-between gap-4' >
                            ...
                            {post.date&& 
                            (<span className='text-[10px] font-mono text-slate-400 bg-slate-100 dark:bg-slate-800/80 px-2 py-1 rounded-md shrink-0 mt-1'>{post.date.split(' ')[0]}
                            </span>
                            )}
                        </div>
                            { post.description && ( <p className='text-sm text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed'> <Highlight text={post.description} query={searchQuery}/>
                            </p>
                        )}
